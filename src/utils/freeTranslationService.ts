...
class FreeTranslationService {
  ...
  async translateText(
    ...
    try {
      const openAiResult = await this.tryOpenAITranslation(
        text,
        targetLanguage,
        sourceLanguage
      )
  ...
  private async tryOpenAITranslation(
    ...
    const translatedText = await this.simulateOpenAITranslation(
      text,
      targetLanguage
    )
  ...
  getStats() {
    return {
      ...
      methodsUsed: {
        openai: Array.from(this.cache.values()).filter(
          (r) => r.method === 'openai'
        ).length,
        browser: Array.from(this.cache.values()).filter(
          (r) => r.method === 'browser'
        ).length,
        community: Array.from(this.cache.values()).filter(
          (r) => r.method === 'community'
        ).length,
      },
    }
  }
