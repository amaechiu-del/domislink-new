...
export async function generateAIResponse(
  ...
  const responses =
    contextResponses[context as keyof typeof contextResponses] ||
    contextResponses.general
...
export async function handleDeploymentCommand(
  ...
  const deploymentCommands = {
    'deploy pricing update':
      '🔄 Pricing structure updated across all DomisLink platforms',
    'deploy feature update':
      '🚀 New features deployed to production successfully',
    ...
    'update marketplace inventory':
      '📦 Marketplace inventory synchronized with new products',
    'deploy real estate listings':
      '🏠 New property listings published successfully',
