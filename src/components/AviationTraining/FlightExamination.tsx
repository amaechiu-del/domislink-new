...
export const FlightExamination: React.FC<FlightExaminationProps> = ({
  ...
  const examinationCriteria: Record<string, ExaminationCriteria[]> = {
    'ppl-check': [
      {
        category: 'Pre-flight Inspection',
        weight: 10,
        score: 0,
        comments: [],
        passed: false,
    ...
    'cpl-check': [
      {
        category: 'Multi-engine Operations',
        weight: 20,
        score: 0,
        comments: [],
        passed: false,
    ...
    'atpl-check': [
      ...
      {
        category: 'High-altitude Operations',
        weight: 15,
        score: 0,
        comments: [],
        passed: false,
  ...
  useEffect(() => {
    setPerformanceScores(
      examinationCriteria[examinationType] || examinationCriteria['ppl-check']
    )
  }, [examinationType])
  ...
  const startExamination = () => {
    ...
    addAiMessage(
      `${userName}, this is your ${getExaminationTitle()} examination. I cannot be disabled during this evaluation. Maintain professional standards.`
    )
    addAiMessage(
      'Beginning with pre-flight inspection. Follow procedures exactly as trained.'
    )
  }
  ...
  const completeExamination = () => {
    ...
    const overallScore = finalScores.reduce(
  ...
  const generateDebriefing = (
    ...
    if (passed) {
      if (score >= 95)
        return `Outstanding performance, ${name}! Your precision and professionalism exceeded standards. You're ready for the next level.`
      if (score >= 90)
        return `Excellent work, ${name}. Your procedures were crisp and decision-making sound. Minor areas for refinement noted.`
      if (score >= 85)
        return `Good performance, ${name}. You demonstrated solid skills with room for improvement in specific areas.`
      return `Satisfactory performance, ${name}. You met the minimum standards but need focused practice to excel.`
    } else {
      if (score >= 70)
        return `${name}, you were close but fell short in critical areas. Additional training required before re-examination.`
      if (score >= 60)
        return `${name}, significant deficiencies noted. You need comprehensive review and practice before attempting this examination again.`
  ...
  const generateRecommendations = (
    ...
    failedCriteria.forEach((criterion) => {
      recommendations.push(
        `Focus on ${criterion.category.toLowerCase()} during your next training session`
      )
    })

    if (recommendations.length === 0) {
      recommendations.push(
        'Continue current training regimen to maintain proficiency'
      )
      recommendations.push(
        'Consider advanced scenarios to challenge your skills further'
      )
    }
            ...
            <ScrollArea className="h-32 bg-slate-900 rounded-lg p-3">
              <div className="space-y-2">
                {aiMessages.map((message, index) => (
                  <div
                    key={index}
                    className="text-sm text-white bg-slate-800 rounded p-2 border-l-2 border-amber-400"
                  >
