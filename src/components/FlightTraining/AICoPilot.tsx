  ...
  useEffect(() => {
    ...
    if (active && messages.length === 0) {
      addMessage({
        type: 'info',
        content:
          'AI Co-Pilot initialized. Ready to assist with flight training.',
        priority: 'low',
      })
    }
  }, [active])
    ...
    if (active && simulationRunning) {
      ...
      if (airspeed > 160) {
        addMessage({
          type: 'warning',
          content:
            'Airspeed high for current configuration. Consider reducing power.',
          priority: 'medium',
        })
      }
          ...
          <div className="space-y-3 p-4">
            {messages.length === 0 ? (
              <div className="text-center text-slate-400 py-8">
                <MessageCircle className="h-8 w-8 mx-auto mb-2" />
                <p>
                  No messages yet. AI Co-Pilot will provide guidance during
                  flight.
                </p>
              </div>
            ) : (
